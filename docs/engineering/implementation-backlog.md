...
**Current status (codebase):** реализовано на базовом уровне —
- в `PracticumCourse` и `PracticumLesson` добавлены индикаторы прогресса по интерактивным шагам (progress bar и счётчик завершённых шагов);
- завершённые уроки и курсы подсвечиваются зелёными карточками с иконкой галочки, заблокированные шаги и уроки имеют явное состояние `locked`;
- на `/practicum` пользователь видит общий базовый курс, свои ролевые треки и может визуально отличить завершённые/незавершённые курсы;
- следующий осмысленный шаг (первый незавершённый урок/курс) выбирается автоматически в кнопках «Продолжить курс» / «Просмотреть курс».
...
**Current status (codebase):** реализовано на базовом уровне —
- вкладка «Прогресс» в `/admin` агрегирует данные `user_progress` и структуры практикума и показывает:
  - обзор по пользователям (количество завершённых интерактивных шагов и курсов, статус общего базового курса, профессиональная роль);
  - обзор по курсам (сколько пользователей начали и завершили каждый курс, тип курса по `course_category`);
  - список «могут быть застрявшими» (пары пользователь–курс с незавершённым курсом и давним последним завершённым шагом, порог по умолчанию ≥ 7 дней);
  - блок недавней активности (последние завершения шагов с указанием курса, урока, типа шага и статуса).
- под агрегатами остаётся таблица сырых записей `user_progress` с join на `profiles` для ручного анализа;
- вся аналитика считается на фронтенде из существующих таблиц (`practicum_courses`, `practicum_lessons`, `practicum_steps`, `user_progress`, `profiles`) без отдельного слоя событий или тяжёлой аналитической подсистемы.
